...
double calculate_norm (Element * A, Element * B, int * enabled_features) {
  double norm = 0.0;
...
void sort_by_norm (Element ** norms) {
  qsort (norms, LINES_NUMBER, sizeof (Element *), compare);
}
...
void teste (Element * E) {
  ...
  for (unsigned int i = 0; i < TRAINING.n_element; ++i) {
    norms[i]->norm = calculate_norm(E, norms[i], enabled);
  }
  free(enabled);
...
void testeGA (Element * E, int * enabled_features) {
  Element ** norms = TRAINING.elements;

  for (unsigned int i = 0; i < TRAINING.n_element; ++i) {
    norms[i]->norm = calculate_norm(E, norms[i], enabled_features);
  }
...
double testing (const char * filename, int length) {
  ...
  fclose(file);
  return 100 * ((double)TRAINING.asserts / (double)TRAINING.n_element);
}


double testingGA (const char * filename, int distance, int * enabled_features) {
